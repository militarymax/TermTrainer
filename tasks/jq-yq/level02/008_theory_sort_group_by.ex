...
📋 **Попробуй**:
1. По цене: `jq '.склад | sort_by(.цена)' зелья.json`
2. Дорогие сначала: `jq '.склад | sort_by(.цена) | reverse' зелья.json`
3. Группировка по типу: `jq '.склад | group_by(.тип) | map({тип: .[0].тип, количество: length})' зелья.json`
4. Экспорт CSV: `jq -r '.склад[] | [.имя, .цена, .тип] | @csv' зелья.json`
...
r1=$(jq '.склад | sort_by(.цена) | .[0].имя' "$DIR/зелья.json" 2>/dev/null)
[ "$r1" = '"лечение"' ] && { echo "✓ sort_by работает"; score=$((score+1)); }

r2=$(jq '[.склад[] | .тип] | unique | length' "$DIR/зелья.json" 2>/dev/null)
