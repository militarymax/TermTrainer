...
📋 **Боевые задания**:
1. Извлеки год без кавычек: `jq -r '.год' бюджет.json`
2. Все названия факультетов: `jq -r '.факультеты[] | .название' бюджет.json`
3. Количество факультетов: `jq '.факультеты | length' бюджет.json`
4. Бюджет магии: `jq '.факультеты[0].бюджет' бюджет.json`
5. Построй отчёт — только названия и студенты: `jq '.факультеты[] | {название, студенты}' бюджет.json`
6. Есть ли профицит: `jq -r '.общий_бюджет.профицит' бюджет.json`
7. Извлеки версию сервера из YAML: `yq '.сервер.версия' конфиг.yaml`
8. Порт БД из YAML: `yq '.база_данных.порт' конфиг.yaml`
9. Конвертируй YAML в JSON и сохрани: `yq -o json '.' конфиг.yaml > конфиг.json`
...
r1=$(jq -r '.год' "$DIR/бюджет.json" 2>/dev/null)
[ "$r1" = "2024" ] && { echo "✓ Год извлечён"; score=$((score+1)); }

r2=$(jq -r '.факультеты[] | .название' "$DIR/бюджет.json" 2>/dev/null | wc -l)
[ "$r2" -ge 4 ] && { echo "✓ Все $r2 факультета"; score=$((score+1)); }

r3=$(jq '.факультеты | length' "$DIR/бюджет.json" 2>/dev/null)
[ "$r3" = "4" ] && { echo "✓ Length корректен"; score=$((score+1)); }

r4=$(yq '.сервер.версия' "$DIR/конфиг.yaml" 2>/dev/null)
[ "$r4" = "v2.1.0" ] && { echo "✓ Версия из YAML"; score=$((score+1)); }

[ -f "$DIR/конфиг.json" ] && { echo "✓ JSON создан"; score=$((score+1)); }
