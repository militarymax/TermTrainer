...
1. **Напиши функцию**:
   `jq 'def прибыль: .доход - .расход; .отделы[] | {название, прибыль: прибыль}' отчёт.json`

2. **Функция с параметрами**:
   `jq 'def рентабельность($prefix): "\($prefix) \((.доход - .расход) / .доход * 100 | floor)%"; .отделы[] | {название, рентабельность: рентабельность("ROI:")}' отчёт.json`
...
r1=$(jq '[.отделы[] | .доход - .расход] | add' "$DIR/отчёт.json" 2>/dev/null)
[ "$r1" = "17000" ] && { echo "✓ Математика работает: прибыль=$r1"; score=$((score+1)); }

if [ -f "$DIR/анализ.jq" ]; then
  r2=$(jq -f "$DIR/анализ.jq" "$DIR/отчёт.json" 2>/dev/null | head -5)
  [ -n "$r2" ] && { echo "✓ Модуль работает"; score=$((score+1)); }
fi

r3=$(jq '.отделы[] | .несуществует // "default"' "$DIR/отчёт.json" 2>/dev/null | head -1)
